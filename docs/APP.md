...
| `src/proxy-server.js` | Express proxy + routing engine (`probeSequential`, `probeParallel`, `learnSuccess`, `learnFailure`, `extractContent`, `recordUsage`) |
...
| `data/UltimateConfig.csv` | **Primary config truth** — `provider,baseURL,apiKeyEnv,model,enabled` (1038 entries, human-editable) |
| `data/proxy-config.json` | Auto-generated from `UltimateConfig.csv` for fast reload — not hand-edited |
| `data/models.csv` | All available models (`provider,model` — 41 rows across 12 providers) for dropdown filtering |
| `data/ProviderConfig.csv` | Provider metadata (`provider,baseURL,apiKeyEnv,modelsEndpoint`) — drives autofill + fetch-models.js |
